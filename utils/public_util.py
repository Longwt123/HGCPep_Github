...
def gen_perturb_output(batch, model, eta=1.0):
    perturb_model = deepcopy(model).cpu()
    for (name, perturb_param), (name, param) in zip(perturb_model.named_parameters(), model.named_parameters()):
        if name.split('.')[0] == 'projection_head':
            perturb_param.data = param.data
        elif name.split('.')[-1] != 'eps':
            perturb_param.data = param.data + eta * torch.normal(0, torch.ones_like(param.data) * param.data.std())
    perturb_model = perturb_model.to('cuda')
    out = perturb_model.forward_cl(batch)
    return out
...
class EarlyStopping(object):
    ...
    # >>> from utils.public_util import EarlyStopping
    #
    # >>> model = nn.Linear(1, 1)
    # >>> criterion = MSELoss()
    ...
    def step(self, score, model, test_score=None, IsMaster=False):
    ...
    def report_final_results(self, i_epoch=None):
        ...
        if self.best_test_score is not None:
            print(f'best_test_score={self.best_test_score:.6f}')
