...
class BaseData:
    ...
    def raw(self, key: str) -> Any:
        ...
        if self.needs_to_load(key):
            ...
            if self._raw.get(key, None) is None:
                ...
                tao_path = '/mnt/sdb/home/lwt/tao/hgnnp_on_RNA/dhg/datasets/PRdata/'
                # file_path_list = [self.data_root / u["filename"] for u in cur_cfg["upon"]]
                file_path_list = [tao_path + u["filename"] for u in cur_cfg["upon"]]
    ...
    def fetch_files(self, files: List[Dict[str, str]]):
        ...
        for file in files:
            ...
            data_path = '/mnt/sdb/home/lwt/tao/hgnnp_on_RNA/dhg/datasets/PRdata/'
            # download_and_check(cur_url, data_path / cur_filename, file["md5"])
